...
function CircleNodes() {
    ...
    let isMounted = true; 
      ...
      async function checkUpdateNodes() {
        ...
        try {
          ...
          if (data.length !== nodesRef.current.length || !isEqual(data, nodesRef.current)) {
  ...
  useEffect(() => {
    {isLoading ? setButtonUpdateText('⏳ Обновляется...') : setButtonUpdateText('🔄 Обновить')}
  }, [isLoading])
  ...
  function renderNodeDetails(node, depth = 0) {
    return (
      <div className={depth > 0 ? 'replica' : ''} key={node.id}>
        {depth === 0 ? <div><button class="delete" onClick={() => handleDeleteNode(node.name, false)}>Удалить</button> <button class="delete" onClick={() => handleDeleteNode(node.name, true)}>Удалить вместе с данными</button></div> : ""}
        <h2>{node.name}</h2>
        <p><strong>Статус:</strong> {getNodeStatus(node.status)}{node.status}</p>
        <p><strong>Кол-во элементов:</strong> {node.items.length}</p>
  ...
  const handleDeleteNode = async (nodeName, isForce) => {
    try {
      const res = await fetch(`api/cluster/nodes/delete/${encodeURIComponent(nodeName)}${isForce ? '?force=true' : ''}`, {
        method: 'DELETE',
      });
  ...
  const onAddNode = async (nodeName, copiesCount) => {
    if (nodeName.length > 0 && copiesCount > 0 && copiesCount <= 10) {
      ...
      try {
        const res = await fetch(`api/cluster/nodes/create/${encodeURIComponent(nodeName)}/${encodeURIComponent(copiesCount)}`, {
          method: 'POST',
        });
        ...
        {[...nodes, { isEmpty: true }].map((node, index) => {
          ...
          return (
            ...
            >
              ...
              <h3>{node.name}</h3>
              <small><strong>Кол-во элементов:</strong> {node.items.length}</small><br />
              <small><strong>ID:</strong> {node.id.slice(0, 6)}...</small><br />
              <small><strong>URL:</strong> {node.url}</small>
            </div>
          );
        })}
